		...
		try:
			...
			if self.event.request.type == 'reportRequest':
				try:
					...
					#Execute plugin
					(self.event.report.raw, self.event.report.verbose, self.event.report.metrics) = self.pluginExecute.do(
					    command=command,
					...
					for evaluator in self.event.request.evaluators:
						(value, status)	= self.calculator.do(
						    output=self.event.report.raw,
						...
						self.event.report.addEvaluator(
						    name=evaluator,
							status=status,
							value=value,
